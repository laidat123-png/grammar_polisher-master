...
class NotificationsBloc extends Bloc<NotificationsEvent, NotificationsState> {
  ...
  NotificationsBloc({
    ...
    on<NotificationsEvent>((event, emit) async {
      await event.map(
        ...
        handleOpenAppFromNotification: (event) =>
            _onHandleOpenAppFromNotification(event, emit),
        clearWordIdFromNotification: (event) =>
            _onClearWordIdFromNotification(event, emit),
        scheduleNextDayReminder: (event) =>
            _scheduleNextDayReminder(event, emit),
        scheduleWordsReminder: (event) => _scheduleWordsReminder(event, emit),
        reminderWordTomorrow: (event) => _reminderWordTomorrow(event, emit),
        getScheduledNotifications: (event) =>
            _onGetScheduledNotifications(event, emit),
        removeScheduledNotifications: (event) =>
            _onRemoveScheduledNotifications(event, emit),
  ...
  _requestPermissions(
      _RequestPermissions event, Emitter<NotificationsState> emit) async {
  ...
  _scheduleNextDayReminder(
      _ScheduleNextDayReminder event, Emitter<NotificationsState> emit) async {
    ...
    if (isGranted) {
      ...
      final scheduledDate = DateTime(now.year, now.month, now.day, 8, 0)
  ...
  _scheduleWordsReminder(
      _ScheduleWordReminder event, Emitter<NotificationsState> emit) async {
    debugPrint('NotificationsBloc: scheduleWordsReminder');
    final isGranted = state.isNotificationsGranted;
    debugPrint(
        'NotificationsBloc: scheduleWordsReminder isGranted: $isGranted');
    if (isGranted) {
      for (int i = 0; i < event.words.length; i++) {
        ...
        await _localNotificationsTools.scheduleNotification(
          id: word.index,
          title: word.word,
          body: word.senses.firstOrNull?.definition ??
        ...
        debugPrint(
            'NotificationsBloc: scheduleWordsReminder scheduledDate: $notificationTime');
      }
    }
  }

  _reminderWordTomorrow(
      _ReminderWordTomorrow event, Emitter<NotificationsState> emit) async {
    ...
    if (isGranted) {
      final pendingNotifications =
          await _localNotificationsTools.pendingNotifications();
      final isScheduled =
          pendingNotifications.any((element) => element.id == event.word.index);
      if (isScheduled) {
        debugPrint(
            'NotificationsBloc: reminderWordTomorrow isScheduled: $isScheduled');
        emit(state.copyWith(
            failure: Failure(message: 'Notification is already scheduled')));
      ...
      await _localNotificationsTools.scheduleNotification(
        ...
        body: event.word.senses.firstOrNull?.definition ??
            'Learn the meaning of this word!',
        scheduledDate: scheduledDate,
        category: NotificationCategory.vocabulary,
      ...
      debugPrint(
          'NotificationsBloc: reminderWordTomorrow scheduledDate: $scheduledDate');
    }
  }

  _onHandleOpenAppFromNotification(_HandleOpenAppFromNotification event,
      Emitter<NotificationsState> emit) async {
    debugPrint('NotificationsBloc: handleOpenAppFromNotification');
    final notification =
        await _localNotificationsTools.getNotificationAppLaunchDetails();
    final notificationPayload =
        notification?.notificationResponse?.payload ?? '';
    final wordId = int.tryParse(notificationPayload);
    if (wordId != null) {
      debugPrint(
          'NotificationsBloc: handleOpenAppFromNotification wordId: $wordId');
      emit(state.copyWith(wordIdFromNotification: wordId));
    }
  }

  _onClearWordIdFromNotification(
      _ClearWordIdFromNotification event, Emitter<NotificationsState> emit) {
    debugPrint('NotificationsBloc: clearWordIdFromNotification');
    emit(state.copyWith(wordIdFromNotification: null));
  }

  _onGetScheduledNotifications(
      _GetScheduledNotifications event, Emitter<NotificationsState> emit) {
  ...
  _onRemoveScheduledNotifications(
      _RemoveScheduledNotifications event, Emitter<NotificationsState> emit) {
    debugPrint('NotificationsBloc: removeScheduledNotifications');
    _localNotificationsTools.cancelNotification(event.id);
    _notificationsRepository
        .removeScheduledNotification(event.id)
        .then((result) {
      if (!isClosed) {
        result.fold(
          (failure) {
            debugPrint(
                'NotificationsBloc: removeScheduledNotifications failure: $failure');
            emit(state.copyWith(failure: failure));
            emit(state.copyWith(failure: null));
          },
          (_) => debugPrint(
              'NotificationsBloc: removeScheduledNotifications success'),
        );
      }
    });
